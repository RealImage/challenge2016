...
func AddSubDistributor() {
	...
	parentDistributor, relation := utils.CheckDistributor(DistributorMap, parentName)
	if relation == "" {
...
func CheckForAccess() {
	...
	distributor, rel := utils.CheckDistributor(DistributorMap, name)
	if rel == "" {
		fmt.Println("Distributor does not exist")
		return
	}
	...
	case "1":
		...
		if utils.HasAccess(distributor, city, "city", LocMap) {
			fmt.Println("YES")
		} else {
			fmt.Println("NO")
		}
	case "2":
		...
		if utils.HasAccess(distributor, state, "state", LocMap) {
			fmt.Println("YES")
		} else {
			fmt.Println("NO")
		}
	case "3":
		...
		if utils.HasAccess(distributor, country, "country", LocMap) {
			fmt.Println("YES")
		} else {
			fmt.Println("NO")
		}
...
out:
	for {
		...
		case "1", "2":
			...
			if isSub {
				Parent := utils.FindParentDistributor(parent, DistributorMap)
				approved := false
				if country != "nil" && country != "" {
					if utils.HasAccess(Parent, country, "country", LocMap) {
						approved = true
					}
				}
				if state != "nil" && state != "" {
					if utils.HasAccess(Parent, state, "state", LocMap) {
						approved = true

					}
				}
				if city != "nil" && city != "" {
					if utils.HasAccess(Parent, city, "city", LocMap) {
						approved = true
					}
				}
				if approved {
