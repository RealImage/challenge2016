...
class DistributorManager
    ...
    def print_status
        distributors.map { |name, distributor| distributor.details }
    end
    ...
    def permitted_to_distribute?(name, type, region_code)
        ...
        distributor_permitted = send("check_for_#{type.to_s}_exclusion", distributor, region_code) && send("check_for_#{type.to_s}_inclusion", distributor, region_code) 
